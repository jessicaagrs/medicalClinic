  ...
  return (
    <div className="flex flex-col items-center justify-center w-full gap-16">
      <div className="flex gap-8 items-center justify-center mt-10">
        ...
        <span
          className={`rounded-full w-5 h-5 ${selectedTab === TabNames.PERSONAL ? 'bg-custom10' : 'bg-custom40'}`}
        ></span>
        <hr className="bg-custom40 h-0 w-16" />
        <span
      ...
      {selectedTab === TabNames.TYPE && (
        <h1>Por favor, selecione o tipo do cadastro</h1>
      )}
      {selectedTab === TabNames.PERSONAL && (
        <h1>Agora, informe alguns dados básicos para seu cadastro</h1>
      )}
      {selectedTab === TabNames.ADDRESS && (
        <h1>Por fim, informe seu endereço</h1>
      )}
    </div>
  );
};
